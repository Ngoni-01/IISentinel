...
def export_pdf():
    ...
    if recent:
        ...
        for did,s in sorted(recent.items(),key=lambda x:x[1])[:20]:
            ...
            ttf_str=(f'{ttf}min' if ttf is not None and ttf<60 else f'{ttf//60}h {ttf%60}m' if ttf is not None else 'N/A')
...
def _poll_node(node_id):
    ...
    with _nodes_lock:
        if node_id not in _nodes: return
        _nodes[node_id].update({'status':status,'latency_ms':latency,'loss_pct':loss_pct,'last_check':now,'health_score':health})
    ...
    try:
        ...
        rec={'device_id':did,'device_type':'router' if sector=='net' else 'base_station' if sector=='tc' else 'sensor',
             ...
             'ai_diagnosis':(f'Node {label} unreachable -- 100% packet loss.' if not reachable else (f'High latency {latency}ms on probe.' if (latency or 0)>120 else None)),
...
if __name__=='__main__':
  ...
  +------------------------------------------------------+""")
    if os.environ.get('DEMO_MODE','false').lower()=='true':
        print('  [DEMO MODE] 15 devices, 4 sites -- auto-injecting data')
